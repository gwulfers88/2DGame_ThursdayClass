...
void GameManager::LoadContent(int8* filename)
{
	...
	while(!file.eof())
	{
		...
		else if (!COM_strcmp(obj.type, "chain") && enemyIndex < numEnemies)
		{
			temp = (Enemy*)MemAllocName(sizeof(Enemy), obj.name);
			new(temp) Enemy;	//Initialize the class constructor.
			Enemy* e = (Enemy*)temp;
		...
		else if (!COM_strcmp(obj.type, "chain"))
		{
			temp->dims = obj.dims;
		}
		else
		...
		std::string meta;
		getline(file, meta);
		ApplyMetadata(temp, meta);

		entities.Insert(temp, obj.name);	//insert into list
...
void GameManager::ApplyMetadata(Entity *object, std::string metadata)
{
	...
	for (size_t i = 0; i < data.size(); i += 4)
	{
		if (data[i] == "follow")
		{
			...
			object->target = entities.GetByName(target);
		...
		{
			std::string target = data[i + 1];
			Entity* obj = entities.GetByName(target);
			obj->child = object;
			object->parent = obj;
			object->collidesWithWorld = false;
			object->isChild = true;
			object->pos = obj->pos + Vec2(10, 0);
			object->pivot = obj->pos + obj->dims;

			//obj->pivot = {0, 0};
			obj->pivot.x = std::stof(data[i + 2]);
			obj->pivot.y = std::stof(data[i + 3]);

			obj->childOffset = object->pos - obj->pivot + object->center;
			//obj->child->pos = obj->pos;

			obj->pivot = obj->pivot - (object->pos/* + object->center*/);
		}
	}
}
	...
	if(entities.GetCount() > 0)
	{
		...
		for(uint32 i = 0; i < entities.GetCount(); i++)
		{
			...
			for(int j = i + 1; j < entities.GetCount(); j++)
			{
				...
				if(obj->layer == obj2->layer || !COM_strcmp(obj2->type, "chain"))
				{
					//Collision Testing
